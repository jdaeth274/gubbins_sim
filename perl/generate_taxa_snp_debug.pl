...
while (scalar(keys %taxa) < $n_taxa) {
	foreach my $taxon (keys %taxa) {
		...
		$diverge{$taxon}++;
		if ($rec_prob < 1) {  # best keep < 1
			my $score = rand(1);
			if ($score <= $rec_prob) {
				$taxa{$taxon} = hom_rec($taxon);		# introduce a homologous recombination. Function selects random start and length and replacing ancestor with donor
			}
		} else {
			for (my $p = 1; $p <= $rec_prob**2; $p++) { # If the rec prod is > 1 square
				my $score = rand($rec_prob**2);
				if ($score <= $rec_prob) {
...
foreach my $taxon (keys %taxa) {
	...
	my $score = rand(1);
	if ($score <= $rec_prob) {
		$taxa{$taxon} = hom_rec($taxon);		# introduce a homologous recombination
	}
	$score = rand(1);
	if ($score <= $ins_prob) {
		phage_ins($taxon);		# introduce a phage into a sequence
...
sub hom_rec {
	...
	if ($excluded == 0) {
		...
		if ($index > 0) {
			...
			@{$recsnp{$e_num}} = rec_snps($old,$donor_seg,$translate{$start}, $taxon, $donor); # This must be where the recombination snps are calculated
			$e_num++;
			undef($old);
...
sub rec_snps {
	...
	my $iso = shift; # The name of the iso to be printed out
	my $strep = shift; #  The name of the donor to be printed out
	my @recip = split(//,$recip); # Create an array from the sequence
	my @donor = split(//,$donor); # Create an array from the donor sequence
	my @snp;
	open SNP_FILE, "> $iso"."-"."$strep"."_snps" or print STDERR "Cannot open output file $outfile.aln!\n" and croak() and exit(1);
	for (my $i = 0; $i <= $#donor; $i++) { # go from the length of the donor in this for loop
		if ($recip[$i] ne $donor[$i] && $recip[$i] ne '-' && $donor[$i] ne '-') { # This takes into account the gaps present in donor and recipient and not counting them as SNPs, so where is this going wrong
			print SNP_FILE "$i "."$recip[$i]"."-"."$recip[$i]"."\n";
			push(@snp,$back_translate{($i+$pos+1)}); # Is the original sequence just really divergent to the one we see in the alignment?
		}
	}
	close SNP_FILE;
	print "I'm in the rec snps bit \n";
	open REC_SEQ, "> $iso"."-"."$strep"."_sequence" or print STDERR "Cannot open output file $outfile.aln!\n" and croak() and exit(1);
	print REC_SEQ ">initial"."\n"."$recip"."\n".">donor"."\n"."$donor"."\n";
	close REC_SEQ;
