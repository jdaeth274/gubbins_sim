...
open SAMPLES, "> $outfile.sample_time.tsv" or print STDERR "Cannot open output file $outfile.sample_time!\n" and croak() and exit(1);
...
while (scalar(keys %taxa) < $n_taxa) {
	## Need to insert some steps here to:
	## - Check if a taxa has been sampled - if so skip to next taxa
	## - whether to sample a taxa and arrest its mutation and recombination - do this here so it hasn't gone through the mutation and recombination 
	##   steps of this time step. 
	$time_step++;
	foreach my $taxon (keys %taxa) {
		if (exists $sampled_isolates{$taxon}){
			next;
		}
		
		$taxa{$taxon} = mutate($taxon);		# introduce a point mutation into every sequence at every timestep
		$diverge{$taxon}++;
		my $rec_score = random_poisson(1, $rec_prob);
		if ($rec_score > 0) {
			my $rec_counter = 0;
			while ($rec_counter < $rec_score){
		...
		if (scalar(keys %taxa) > 10 && scalar(keys %taxa) > (scalar(keys %sampled_isolates) + 2)){
			my $sampler = rand(1);
			if ($sampler <= $sampled_prob){
				# Add the taxon ID and time_step to another data structure for later printing
        		# Assuming you have a data structure called @to_print for this purpose
        		push @to_print, "taxon_${taxon}\t$time_step";
            print STDERR "Sampling an isolate\n";
        		# Add the taxon to %sampled_isolates
        		$sampled_isolates{$taxon} = 1;
				next;
			}
		}

		my $branch_score = rand(1);
		if ($branch_score <= $branch_prob) { # Whether a new lineage is formed, sampling density, lower score more SNPs accumulate before detect branching
			$index++;
...
## Print out sampling times 
print SAMPLES "Taxa\tDate\n";
foreach my $entry (@to_print) {
    print SAMPLES "$entry\n";
}
close SAMPLES;
