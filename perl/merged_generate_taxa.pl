...
unless ($min_rec_length >= 0) {
    print STDERR "The minimum recombination length must be >=0\n";
    croak();
    exit(1);
}

unless ($sub_model eq "JC" || $sub_model eq "GTR") {
    print STDERR "Substitution model must be JC or GTR\n";
    croak();
    exit(1);
}
...
while (scalar(keys %taxa) < $n_taxa) {
    foreach my $taxon (keys %taxa) {
        # introduce a point mutation into every sequence at every timestep
        if ($sub_model eq "JC") {
            $taxa{$taxon} = mutate_jc($taxon);
        } elsif ($sub_model eq "GTR") {
            $taxa{$taxon} = mutate_gtr($taxon);
        } else {
            print STDERR "Unknown substitution model $sub_model\n";
            exit(1);
        }
        $diverge{$taxon}++;
        ...
        if ($score > 0) {
            ...
            while ($rec_counter < $score){
                $taxa{$taxon} = hom_rec($taxon,$min_rec_length);
                $rec_counter += 1;
...
sub mutate_gtr {
    ...
    ## Take the rate matrix from the R script GTR_rate_matrix form
    my $rand_base = rand();
    my $der_base = "N";
    
    if($anc_base eq "A") {
        if($rand_base <= 0.1595932){
            $der_base = "C";
        }elsif($rand_base <= 0.8312907){
            $der_base = "G";
        }elsif($rand_base <= 1.00){
            $der_base = "T";
        }
    }elsif($anc_base eq "C"){
        if($rand_base <= 0.1744883){
            $der_base = "A";
        }elsif($rand_base <= 0.2485140){
            $der_base = "G";
        }elsif($rand_base <= 1.00){
            $der_base = "T";
        }
    }elsif($anc_base eq "G"){
        if($rand_base <= 0.7442038){
            $der_base = "A";
        }elsif($rand_base <= 0.8192189){
            $der_base = "C";
        }elsif($rand_base <= 1.00){
            $der_base = "T";
        }
    }elsif($anc_base eq "T"){
        if($rand_base <= 0.1655290){
            $der_base = "A";
        }elsif($rand_base <= 0.8399079){
            $der_base = "C";
        }elsif($rand_base <= 1.00){
            $der_base = "G";
        }
